...
我们看下源码的实现：Spring 如何实现将资源配置（以`xml`配置为例）通过加载，解析，生成`BeanDefination`并注册到 IoC 容器中的（圈出来的部分）。

![](Spring详解—IOC实现原理之IOC初始化流程/spring-framework-ioc-source-73.png)
...
```
通过`AbstractApplicationContext的setParent(parent)`方法将父容器的`Environment`合并到当前容器。
```java
...
![](Spring详解—IOC实现原理之IOC初始化流程/spring-framework-ioc-source-8.png)
...
继续看子类`XmlBeanDefinitionReader的loadBeanDefinitions(Resource …)`方法看到代表`bean`文件的资源定义以后的载入过程。
```java
/**
    * 本质上是加载XML配置的Bean。
...
### 总结
![](Spring详解—IOC实现原理之IOC初始化流程/spring-framework-ioc-source-9.png)

现在通过上面的代码，总结一下IOC容器初始化的基本步骤：
* 初始化的入口在容器实现中的`refresh()`调用来完成
* 对`bean`定义载入 IOC 容器使用的方法是`loadBeanDefinition`，其中的大致过程如下：
 * 通过`ResourceLoader`来完成资源文件位置的定位，`DefaultResourceLoader`是默认的实现，同时上下文本身就给出了`ResourceLoader`的实现，可以从类路径，文件系统, URL 等方式来定为资源位置。如果是`XmlBeanFactory`作为 IOC 容器，那么需要为它指定`bean`定义的资源，也就是说`bean`定义文件时通过抽象成`Resource`来被 IOC 容器处理的
 * 通过`BeanDefinitionReader`来完成定义信息的解析和`Bean`信息的注册, 往往使用的是`XmlBeanDefinitionReader`来解析`bean`的`xml`定义文件 - 实际的处理过程是委托给`BeanDefinitionParserDelegate`来完成的，从而得到`bean`的定义信息，这些信息在 Spring 中使用`BeanDefinition`对象来表示 - 这个名字可以让我们想到`loadBeanDefinition`,`RegisterBeanDefinition`这些相关的方法 - 他们都是为处理`BeanDefinition`服务的
