...
## 获取文件属性
获取文件属性信息的第一步是先创建一个`File`类对象并指向一个已存在的文件， 然后调用`File`的方法进行操作。
```java
public class Test02 {
  public static void main(String[] args) {
...
与`InputStream`类相同，在`Reader`类中也包含`close()、mark()、skip()`和`reset()`等方法，这些方法可以参考`InputStream`类的方法。`Reader`类中的`read()`方法：
...
与`OutputStream`类相同，`Writer`类也包含`close()、flush()`等方法，这些方法可以参考`OutputStream`类的方法。`Writer`类中的`write()`方法和`append()`方法：
