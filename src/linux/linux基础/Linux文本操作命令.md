...
| 命令 << 分界符          | 从标准输入设备（键盘）中读入，直到遇到分界符才停止（读入的数据不包括分界符） |
...
|    命令符号格式	    | 作用                             |
|:-------------:|:-------------------------------|
|   命令 > 文件	    | 将标准输出结果重定向到指定的文件中，清空原有文件数据     |
|   命令 2> 文件	   | 将错误输出结果重定向到指定的文件中，清空原有文件数据     |
|   命令 >> 文件	   | 将标准输出结果重定向输出到指定的文件中，追加到原有内容的后面 |
|  命令 2>> 文件	   | 将错误输出结果重定向到指定的文件中，追加到原有内容的后面   |
| 命令 >> 文件 2>&1 | 将标准输出和错误输出共同写入到指定文件，追加到原有内容的后面 |
...
```
## 管道命令符 |
管道命令符可以将一个命令的输出作为另一个命令的输入，实现不同命令之间的数据传输与处理。
```shell
command1 | command2
```
`command1`的输出会作为`command2`的输入，实现两个命令之间的数据传输。
## sed命令
...
| -f | 将 sed 的动作写在一个文件内，用 –f filename 执行 filenam e内的 sed 动作 |
| -n | 只打印模式匹配的行                                            |
| -i | 直接修改文件内容                                             |
| -r | 支持扩展表达式                                              |
