...
```
上面的操作会在宿主机`/var/lib/docker/volumes`下创建一个数据卷目录与容器内的`/webapp-data`进行映射。
...
注意：如果直接挂载一个文件，很多文件编辑工具，包括 vi 或者`sed --in-place`，可能会造成文件`inode`的改变，这会导致报错误信息。所以最简单的办法就直接挂载文件的父目录。
### 文件拷贝
...
## 数据卷容器
上面的是主机和容器之间共享数据，那么如何你有一些持续更新的数据需要在容器之间共享，最好的方法就是创建数据卷容器(`Data Volume Container`)。数据卷容器，其实就是一个正常的容器，专门用来提供数据卷供其它容器挂载的。
```shell
...
## 参考文章
[Docker基础 - Docker数据卷和数据管理](https://www.pdai.tech/md/devops/docker/docker-06-data.html)
[Docker 数据管理](https://vuepress.mirror.docker-practice.com/data_management/)
