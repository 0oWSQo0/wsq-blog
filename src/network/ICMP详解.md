...
| 类型值 | 内容                             |  种类  |
|:---:|:-------------------------------|:----:|
|  0  | 回送响应(Echo Replay)              | 查询报文 |
|  3  | 目的不可达(Destination Unreachable) | 差错报文 |
|  4  | 源点抑制(Source Quench)            | 差错报文 |
|  5  | 重定向或改变路由(Redirect)             | 差错报文 |
|  8  | 回送请求(Echo Request)             | 查询报文 |
| 11  | 超时(Time Exceeded)              | 差错报文 |
| 12  | 参数问题(Parameter Problem)        | 差错报文 |
| 13  | 时间戳请求(Timestamp Request)       | 查询报文 |
| 14  | 时间戳响应(Timestamp Reply)         | 查询报文 |
...
### 超时消息
IP 包中有一个字段 TTL（`Time To Live`，生存周期），它的值随着每经过一次路由器就会减 1，直到减到 0 时该 IP 包会被丢弃。
...
### 时间戳请求/时间戳响应
时间戳可以记录 ICMP 消息一次往返所需的时间。源设备发送一个带有发送时间的时间戳请求消息，目的设备收到后，发送一个带有原设备发送时间、目的设备接收时间以及目的设备发送时间的时间戳响应消息。源设备收到时间戳响应时，并同时记录到达时间。这些时间戳可以估计网络上的传输时间。
