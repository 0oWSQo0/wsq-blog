...
### SQL的基本书写规则
#### SQL 语句要以分号;结尾
在 RDBMS （关系型数据库）中，SQL 语句是逐条执行的，一条 SQL 语句代表着数据库的一个操作。SQL 语句使用英文分号`;`结尾。
#### SQL 语句不区分大小写
SQL 不区分关键字的大小写。不管写成`SELECT`还是`select`，解释都是一样的。表名和列名也是如此。

插入到表中的数据是区分大小写的。例如，向数据库中插入单词`Computer、COMPUTER`或`computer`，这三个是不一样的数据。
#### 常数的书写方式是固定的
